...
async function fetchMoviePosters(
  ismovie: boolean,
  transformedRecommendations: { id: number; movieTitle: string }[]
) {
...
export async function callGPT(userPreferences: any) {
  try {
    ...
    const recommendationsWithPosters = await fetchMoviePosters(
      isMovie,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function callDatabase(userPreferences: any) {
  try {
    ...
    console.error("Error in callDB", error);
    throw new Error(
      "An error occurred while fetching recommendations. Please try again later."
    );
  }
