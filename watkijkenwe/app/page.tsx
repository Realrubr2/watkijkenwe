              ...
              <div className="bg-gray-100 p-4 rounded-md shadow-md">
                <ul className="space-y-4 text-left text-lg">
                  {recommendation.recommendations.map((rec, index) => (
                    <li key={index} className="bg-white p-4 rounded-md flex flex-col md:flex-row items-center md:items-start space-x-4 shadow-sm">
                      <div className="flex-grow text-center md:text-left">
                        <strong className="block">{rec.title}</strong> {/* Ensures title is on its own line */}
                        {isMobile && (
                          <button 
                            onClick={() => toggleExpand(index)} 
                            className="text-blue-500 underline mt-2 block"
                          >
                            {expandedIndex === index ? "Verberg details" : "Toon details"}
                          </button>
                        )}
